...
export default function App() {
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'extract':      return <AudioExtractionPanel key="extract" />;
      case 'audioToSub':   return <TranscriptionPanel key="audioToSub" />;
      case 'videoToSub':   return <VideoToSubPanel key="videoToSub" />;
      case 'queue':        return <QueuePanel key="queue" />;
      default:             return <AudioExtractionPanel key="extract" />;
    }
  };

  return (
    <div className="h-full flex flex-col bg-[var(--bg-primary)]">
      {/* Title bar - background spans full width behind overlay */}
      ...
      {/* Separator line below title bar - full width */}
